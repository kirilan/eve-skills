...
| `market` | Live order book for any type: best sell/buy, spread, margin, listed volume - per region, at a station-level trade hub, or across the cluster with `--global`; `--depth N` adds the top N sells and buys per scope; `--history DAYS` adds traded volume; `--group`/`--category NAME` prices everything in one of them, `--fields a,b,c` picks the columns; `--seller CHAR` says what one character keeps per unit after CCP's two cuts and whether listing beats selling now | no for the prices — public ESI, plus local SDE data (`update-data`) to expand a group name; `--seller` needs that character logged in, and its standing with the station's owner comes from the optional `standings` consent |
| `sell-plan` | Allocate explicit items or inventory among hub stations by fee-adjusted uplift, regional volume/day and book saturation; includes routes, Daytrading repricing reach and all-reference comparison | stored seller's core skills consent; optional `standings` for NPC broker fees with standings, `assets` for `--from-inventory`, `divisions` for named corporation hangar filters |
